...
export class DocumentosService {
  ...
  readonly uploadStatus = signal<Record<DocTipo, Status | null>>({ empresa: null, centro: null, proyecto: null });
  ...
  subir(
    ...
    if (file.type !== 'application/pdf') {
      this.setUploadStatus(tipo, { type: 'error', text: 'Solo se permiten archivos PDF' });
      return;
    }
    ...
    this.http.post(this.api.url('/documentos/upload'), form).subscribe({
      next: () => {
        this.setUploadStatus(tipo, { type: 'ok', text: `${file.name} cargado exitosamente` });
