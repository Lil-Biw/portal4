...
- ✅ ~~**Cross-tenant leak** en documentos de centros/proyectos~~ — **SOLUCIONADO (2026-08-12)**: `centros-costos.service.ts`/`proyectos.service.ts` validan que `centroId`/`proyectoId` pertenezcan a `empresaId` (`autorizarCentro`/`autorizarProyecto`, mismo patrón que `validarPertenencia` en actividades) antes de servir datos o documentos. De paso se cerró el filtrado por `centros_asignados` (un usuario `rol: 'usuario'` ya no ve centros/proyectos de su propia empresa fuera de los que tiene asignados) y se le agregó `@Roles('super_admin','admin_smartclarity')` a `PermisosController` (`/permisos`), que no tenía ningún guard de rol. Test de regresión: `back4/scripts/test-permisos-seguridad.ts` (`npm run test:permisos-seguridad`).
...
- **Editar/eliminar actividades en perfil consumidor (rol `usuario`):** además del permiso del catálogo (`@RequiereAccion('actividades', 'editar'|'eliminar')`, que sigue siendo el interruptor), el PUT/DELETE del controller nested pasan por `autorizarModificacion()`, que exige: (1) el centro de la actividad esté en `centros_asignados` del usuario (y si la mueve de centro, el destino también, y de su empresa), (2) la actividad la haya creado él mismo u otro usuario `rol: 'usuario'` de su empresa — las creadas por admins Eclarity quedan protegidas (`ForbiddenException`). `GET /empresas/:id/actividades` adjunta por actividad los flags `puede_editar`/`puede_eliminar` para que el frontend muestre/oculte botones sin duplicar la lógica.
